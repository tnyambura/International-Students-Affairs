...
class DashboardController extends Controller
{
    ...
    public function GetAllbookedMeeting(){
        ...
        if(sizeOf($data) > 0){
            $data=collect($data)->sortByDesc('booked_date_time');
    ...
    public function index(Request $request){
        ...
        if(!Auth::user()){
            ...
            $passGt =  DB::table('users')->select('password')->where('id',Auth::user()->id)->limit(1)->get()[0]->password;
            if(Hash::check('123456',$passGt)){
                return redirect('/password/replace');
            }else{
                if(sizeOf($fetcher) > 0){
                    $is_buddy=false;
